...
int main() {    
    ...
    Logger::getInstance().log(LogLevel::WARNING, VERSION_INFO);
    ...
    if (launcherPath->empty()) {
        ...
        if (selectedPath) {
            ...
            // Save the launcher path directly to config JSON
            config.getConfig()["launcherPath"] = pathStr;
            config.getConfig()["injectionMethod"] = "loadLibrary";
            config.save();
        }
        else {
