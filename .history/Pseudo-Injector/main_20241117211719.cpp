...
int main() {
    ...
    if (launcherPath->empty()) {
        ...
        if (selectedPath) {
            ...
            launcherPath = std::wstring_convert<std::codecvt_utf8<wchar_t>>().to_bytes(wPath);
            Config::config["/launcherPath"] = *launcherPath;
            Config::config["/injectionMethod"] = "loadLibrary";  // Set default injection method
            Config::actualSave();
